...
fun HomeScreenPreferences(
    ...
    PreferenceLayout(
        ...
        val customBgPath by prefs2.customBackgroundImagePath.getAdapter()
            ...
            scope.launch {
                withContext(Dispatchers.IO) {
                    try {
                        ...
                        val destFile = File(backgroundDir, "custom_background")
                        context.contentResolver.openInputStream(uri)?.use { inputStream ->
                            FileOutputStream(destFile).use { outputStream ->
                                inputStream.copyTo(outputStream)
                            }
                        }
                        prefs2.customBackgroundImagePath.set(destFile.absolutePath)
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
        ...
        PreferenceGroup(heading = stringResource(id = R.string.wallpaper)) {
            ...
            ClickablePreference(
                label = stringResource(id = R.string.launcher_background_image_set),
                onClick = {
            ...
            if (customBgPath.isNotBlank()) {
                ClickablePreference(
                    label = stringResource(id = R.string.launcher_background_image_clear),
                    onClick = {
                        scope.launch {
                            withContext(Dispatchers.IO) {
                            ...
                            prefs2.customBackgroundImagePath.set("")
                        }
                    },
                )
            }
